...
suspect_person_id AS (
  SELECT person_id
  FROM bank_acc
  WHERE acc_number IN suspect_account_number
),
suspect_license_num_plate AS (
  SELECT license_num_plate
  FROM courthouse_security_logs
  WHERE month = 7 AND day = 28 AND hour = 10
  AND minute BETWEEN 10 AND 20
),
suspect_phone_call AS (
  SELECT caller
  FROM phone_call
  WHERE month = 7 AND day = 28
)
...
AND license_num_plate IN suspect_license_num_plate
AND phone_number IN suspect_phone_call;
/*
...
WITH earliest_flight_id AS (
  ...
  AND origin_airport_id = (SELECT id FROM airport WHERE city = "Fiftyville")
  ORDER BY hour
  LIMIT 1
)
SELECT * FROM airport WHERE id IN earliest_flight_id;
/*
...
/* reference: https://github.com/chinhungtseng/cs50x2021/blob/master/week7/pset7/fiftyville/answers.txt /*
